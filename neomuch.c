...
static notmuch_database_t *db;
static notmuch_query_t *q;
...
static unsigned int msg_unread;
...
static const char *fmt_subj(const char *subj);
static int utf8_width(const char *str);
static void nm_query(void);
static void nm_unread(void);
static void nm_db_read(void);
static void nm_db_clean(void);
...
static void nm_db_clean(void)
{
	...
	if (db) { notmuch_database_destroy(db); db = NULL; }
}
...
static const char *fmt_from(const char *from)
{
	...
	while (len > 0 && (from[len - 1] == ' ' || from[len - 1] == '"')) len--;
...
{
	if (j == 0) return NULL;
	for (int k = j + 1; k < threads[i].msgs; k++)
		if (strcmp(threads[i].msg[j].parent_id, threads[i].msg[k].parent_id) == 0)
			return "├─>";
	return "└─>";
}
...
static const char *fmt_subj(const char *subj)
{
	static char buf[128];
	size_t i = 0, j = 0;
	while (subj[i] && j < sizeof(buf) - 1)
	{
		if ((unsigned char)subj[i] == 0xEF &&
		        (unsigned char)subj[i + 1] == 0xB8 &&
		        ((unsigned char)subj[i + 2] == 0x8E ||
		         (unsigned char)subj[i + 2] == 0x8F))
		{ i += 3; continue; }
		buf[j++] = subj[i++];
	}
	buf[j] = '\0';
...
static void nm_query(void)
{
	...
	notmuch_threads_t *t_iter;
	ret = notmuch_query_search_threads(q, &t_iter); if (ret != 0) return;

	size_t i = 0;
	while (notmuch_threads_valid(t_iter) && i < thr_count)
	{
		notmuch_thread_t *t = notmuch_threads_get(t_iter);
		if (!t) { notmuch_threads_move_to_next(t_iter); continue; }
		...
		while (allmsgs && notmuch_messages_valid(allmsgs) && m_idx < MAX_MSGS_PER_THREAD)
		{
			...
			dst->parent_id = notmuch_message_get_header(m, "in-reply-to");
		...
		notmuch_threads_move_to_next(t_iter);
	}
}
...
{
	char buf[64];
	if (strcmp(qstr[qid], "*") == 0)
		snprintf(buf, sizeof(buf), "tag:unread");
	else
		snprintf(buf, sizeof(buf), "%s and tag:unread", qstr[qid]);
	notmuch_query_t *q_unread = notmuch_query_create(db, buf);
	notmuch_query_count_messages(q_unread, &msg_unread);
	notmuch_query_destroy(q_unread);
}
...
static void render(void)
{
	...
	for (unsigned int i = 0; i < thr_count; i++)
	{
		...
		{
			rows++; y = rows - page * page_height;
			if (y == 0) y++;
			struct msg *m = &t->msg[j];
			const char *date = fmt_date(m->date);
			const char *from = fmt_from(m->from);
			const char *tree = fmt_tree(i, j);
			const char *subj = fmt_subj(m->subj);
			if (t->unfold && j > 0 && !strcmp(m->subj, t->msg[0].subj)) subj = NULL;
			int x = 0;
...
int main(void)
{
	...
	while (1)
	{
		...
		if (ev.type == TB_EVENT_KEY)
		{
			...
			case KEY_Q_NEXT: sel = 0; qid++; nm_query(); break;
			case KEY_Q_PREV: sel = 0; qid--; nm_query(); break;
